...
use crate::items::weapons::factories::specific::spears::LeveledSpearGenerator;
use crate::items::weapons::factories::WeaponGenerator;

#[derive(Clone)]
pub struct ArmedSpearProcessor;

impl ArmedSpearProcessor {}

impl Default for ArmedSpearProcessor {
    fn default() -> Self {
        Self {}
    }
}

impl LeveledCreatureProcessor for ArmedSpearProcessor {
    fn process(&self, world: &mut World, creature_entity: Entity, level: BuildLevel) -> Entity {
        {
            let weapon_entity = LeveledSpearGenerator::default().create(world, level);
